...
static void fill(float *out, int w, int h, float freq, int32_t seed) {
    ...
    for (int py = 0; py < h; py++) {
        ...
        for (; px + 7 < w; px += 8) {
            ...
            float32x4_t vx1 = vaddq_f32(vx0, vdupq_n_f32(4.0f*freq));  /* saves 3 muls */
            ...
            float32x4_t i1x0=vbslq_f32(vcgtq_f32(x00,y00),C1,zero);
            float32x4_t i1y0=vsubq_f32(C1,i1x0);
            float32x4_t i1x1=vbslq_f32(vcgtq_f32(x01,y01),C1,zero);
            ...
            {float32x4_t gx,gy;
             ...
             grad(h1,&gx,&gy);
             CORNER(x00,y00,n0) CORNER(x01,y01,n1)}
            /* Corner 1 */
            {float32x4_t x10=vaddq_f32(vsubq_f32(x00,i1x0),Cx), y10=vaddq_f32(vsubq_f32(y00,i1y0),Cx);
             ...
             float32x4_t gx,gy; grad(h0,&gx,&gy); CORNER(x10,y10,n0)
             grad(h1,&gx,&gy); CORNER(x11,y11,n1)}
            /* Corner 2 */
            {float32x4_t x20=vaddq_f32(vsubq_f32(x00,C1),Cx2), y20=vaddq_f32(vsubq_f32(y00,C1),Cx2);
             ...
             uint32x4_t h1=hash2d4(vaddq_s32(xi1,one_i),vaddq_s32(yi1,one_i),sv);
             float32x4_t gx,gy; grad(h0,&gx,&gy); CORNER(x20,y20,n0)
             grad(h1,&gx,&gy); CORNER(x21,y21,n1)}
            #undef CORNER
            /* Combined normalization: 0.5 + n * 46.432 */
